...
The chapters tell one story. Each chapter ends with a limitation, and the next
chapter's concept is the answer to it. The order below follows that story, not the
order the code was first written in. The number in brackets is the operation number
from `docs/harnesses/OPERATIONS.md`.

The **Explained** column lists the points the chapter document must teach, a few
words each. It is the checklist for whoever writes the chapter.

| Ch | Concept | Explained | What the reader sees at the end |
|---|---|---|---|
| 0 | Project setup | one tool for the environment (uv); secrets in `.env`, never in git; tests run offline | `just setup`, `just test` green |
| 1 | One raw model call [1] | a request is plain HTTP; the key travels in a header; the request is stateless: nothing is kept between calls; the reply is a list of typed blocks (reasoning, text) | `just ch01` prints `pong` |
...
| 8 | Token accounting and cost [15] | usage numbers come back with each reply; input versus output tokens; a price table per model; running total per session; growth from chapter 2 now has a price | cost shown after each turn |
...
| 16 | Outside tool servers, MCP [22] | tools that live in another process; discover them at start; the same loop from chapter 4 runs them | tools from a Model Context Protocol server |
...
Chapters 0 to 3 already have code. Chapters 2 and 3 swap places compared with the
current `docs/chapters/` files and `just` recipes, so one small task renames those
before tagging. Chapter 2 is taught with a plain message list first; the graph and
checkpointer arrive in chapter 3 as the tidy way to keep that list.
