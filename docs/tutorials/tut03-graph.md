...
- **One graph node is one turn.** You send a message; one node prefixes the system prompt and
  answers. The reply joins the thread. One node now leaves room for tutorial 4 tool-node edges. The
...
- **A checkpointer keeps the list per thread id.** In tutorial 2 the list was a variable; the
...
- **Tokens arrive one by one, so you print them as they come.** In tutorial 2 replies arrived in one
  ...
  a restart still loses the list, fixed in tutorial 7.
